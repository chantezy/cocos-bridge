...
function getProjectIdentity(projectPath = getProjectPath()) {
  return crypto
    .createHash('sha256')
    .update(`cocos-bridge:${normalizeProjectIdentityPath(projectPath)}`)
    .digest('hex')
    .slice(0, 24);
}
...
function loadConfig() {
  ...
  const configPath = path.join(projectPath, 'cocos-bridge.config.json');
  const fileConfig = loadJson(configPath) || {};

  return {
    ...DEFAULTS,
    ...fileConfig,
    host: process.env.COCOS_BRIDGE_HOST || fileConfig.host || DEFAULTS.host,
    port: clampPort(process.env.COCOS_BRIDGE_PORT || fileConfig.port || DEFAULTS.port),
    toolProfile: normalizeProfile(process.env.COCOS_BRIDGE_PROFILE || fileConfig.toolProfile || DEFAULTS.toolProfile),
