...
const LOG_PREFIX = '[Cocos Bridge Server]';
...
class McpServer {
  ...
  async tryAttachToExisting(port) {
    ...
    if (!probe || !probe.result) {
      this.log('warn', `Port ${port} is occupied, but no compatible Cocos Bridge MCP initialize response was received.`);
      return false;
    }

    const result = probe.result || {};
    const serverInfo = result.serverInfo || {};
    const bridge = result.cocosBridge || result.funplay || {};
    const remoteProjectIdentity = bridge.projectIdentity || serverInfo.projectIdentity || '';
    ...
    if (remoteName === this.serverName && remoteProjectIdentity === this.projectIdentity) {
      ...
      this.attachedInfo = {
        ...
        projectName: bridge.projectName || this.projectName,
  ...
  probeExistingServer(port) {
    const body = JSON.stringify({
      jsonrpc: '2.0',
      id: 'cocos-bridge-probe',
      method: 'initialize',
      params: {
        protocolVersion: MCP_PROTOCOL_VERSION,
        clientInfo: {
          name: 'cocos-bridge-probe',
          version: this.serverVersion,
        },
      },
    });
