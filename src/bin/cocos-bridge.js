...
async function main() {
  ...
  if (hasFlag(args, '--version')) {
    console.error(`cocos-bridge ${readPackageVersion()}`);
    return 0;
  }

  const urlText = getOption(args, '--url')
    || process.env.COCOS_BRIDGE_URL
    || DEFAULT_URL;
  const timeoutText = getOption(args, '--timeout-seconds')
    || process.env.COCOS_BRIDGE_TIMEOUT_SECONDS
  ...
  console.error(`[Cocos Bridge] Bridging stdio to ${endpoint.href}`);
...
async function bridgeStdioToHttp({ input, output, endpoint, timeoutMs }) {
  ...
  while (true) {
    ...
    try {
      ...
      console.error(`[Cocos Bridge] ${error.message}`);
      if (requestId !== null) {
...
function printHelp() {
  ...
  console.error('  COCOS_BRIDGE_URL');
  console.error('  COCOS_BRIDGE_TIMEOUT_SECONDS');
}
